...
app.post('/meeting', async function (req, res) {
  ...
  const jwtToken = 'eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOm51bGwsImlzcyI6IlFfeWN3UGFzUmRLR2FjaWgzSkRqUVEiLCJleHAiOjE2NzkyMDc5MjUsImlhdCI6MTY3OTIwMjUyNX0.xNOmYc5i5A1vQEDGZQz_nyu5_xwi2IZbgYdQUN9HMts';

  try {
    const response = await axios.post(zoomApiUrl, {
      topic,
      type: 2,
      start_time,
      duration,
...
socketIO.use(function (socket, next) {
  ...
  if (socket.handshake.query && socket.handshake.query.token) {
    jwt.verify(socket.handshake.query.token, config.secret_key, function (err, decoded) {
...
socketIO.on('connection', async (socket) => {
  ...
  socket.on('message', async data => {
    ...
    if (data.type) {
      ...
      message.save(function (err, user) {
        let getAllmessage = MessageModal.find({ roomId: data.roomId }, function (err, obj) {
      ...
      let getAllmessage = MessageModal
        .find({ roomId: data.roomId }, function (err, obj) {
          socket.emit("message", obj);
          socket.broadcast.emit("message", obj);
        })
        .populate("taskId")
  ...
  socket.on('disconnect', async () => {
    ...
    let updateoffline = await UserModel.findOneAndUpdate({ SocketId: socket.id }, { Status: "offline" })
    // if (updateoffline) {
    //   socket.disconnect()
    // }
    // let users = await userModel.find({ currentStatus: "online" });
...
try {
  ...
  mongoose.connect('mongodb+srv://jameel86:YGKx17uttjwe8knk@cluster0.zpiaagb.mongodb.net/quoded?retryWrites=true&w=majority');
